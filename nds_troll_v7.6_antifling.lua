--[[
    ╔═══════════════════════════════════════════════════════════════════════════╗
    ║                     NDS TROLL HUB v7.6 - ANTI-FLING & BODY FLING                         ║
...
-- FUNÇÃO ANTI-FLING - Proteção completa contra ser arremessado
local function ToggleAntiFling()
    ...
    if State.AntiFling then
        ...
        -- Método 1: Desabilitar colisão de players com velocidade alta (PreSimulation)
        Connections.AntiFlingPre = RunService.PreSimulation:Connect(function()
            ...
            for _, player in pairs(Players:GetPlayers()) do
                ...
                -- Verificar velocidade do player
                local linearVel = rootPart.AssemblyLinearVelocity
                local angularVel = rootPart.AssemblyAngularVelocity
                
                -- Se o player está se movendo muito rápido (tentando fling)
                if linearVel.Magnitude > 60 or angularVel.Magnitude > 60 then
                    -- Desabilitar colisão de TODAS as partes do player atacante
        ...
        -- Método 2: Limitar própria velocidade e salvar posição segura (Stepped)
        Connections.AntiFlingStep = RunService.Stepped:Connect(function()
            if not State.AntiFling then return end
            
            local hrp = GetHRP()
            if not hrp then return end
            
            local linearVel = hrp.AssemblyLinearVelocity
            local angularVel = hrp.AssemblyAngularVelocity
            
            -- Limitar velocidade excessiva (sendo arremessado)
            if linearVel.Magnitude > 150 then
                hrp.AssemblyLinearVelocity = Vector3.zero
            end
            if angularVel.Magnitude > 80 then
                hrp.AssemblyAngularVelocity = Vector3.zero
            end
            
            -- Salvar posição segura quando velocidade é baixa
            if linearVel.Magnitude < 40 and angularVel.Magnitude < 30 then
                lastSafePosition = hrp.CFrame
                lastSafeTime = tick()
            end
        end)
        
        -- Método 3: Teleportar de volta se for arremessado muito longe (Heartbeat)
        Connections.AntiFlingHeart = RunService.Heartbeat:Connect(function()
            if not State.AntiFling then return end
            ...
            -- Se foi arremessado muito longe da posição segura
            local distance = (hrp.Position - lastSafePosition.Position).Magnitude
            if distance > 250 and (tick() - lastSafeTime) > 0.5 then
            ...
            if hrp.Position.Y < -100 or hrp.Position.Y > 500 then
        ...
        -- Método 4: Desabilitar estados de Ragdoll
        ...
        Notify("Anti-Fling", "Proteção ativada!", 3)
        return true, "Anti-Fling ativado"
    else
        ClearConnections("AntiFling")
        
        -- Reabilitar estados de Ragdoll
        local humanoid = GetHumanoid()
        if humanoid then
            pcall(function()
        ...
        return false, "Anti-Fling desativado"
    end
end

-- FUNÇÃO BODY FLING ATTACK - Usa seu próprio corpo para causar fling (funciona contra Noclip)
local function ToggleBodyFlingAttack()
    State.BodyFlingAttack = not State.BodyFlingAttack
    
    if State.BodyFlingAttack then
        if not State.SelectedPlayer or not State.SelectedPlayer.Character then
            State.BodyFlingAttack = false
            return false, "Selecione um player!"
        end
        ...
        -- Adicionar BodyThrust para força extra
        local thrust = Instance.new("BodyThrust")
        thrust.Name = "_NDSBodyThrust"
        thrust.Force = Vector3.new(9999999, 0, 9999999)
        thrust.Parent = myHRP
        table.insert(CreatedObjects, thrust)
        ...
        -- Loop principal de Body Fling
        Connections.BodyFlingMain = RunService.Heartbeat:Connect(function()
            if not State.BodyFlingAttack then return end
            ...
            -- Teleportar para perto do alvo com offset aleatório
            local offset = Vector3.new(
                math.random(-3, 3),
                math.random(-1, 1),
                math.random(-3, 3)
            ...
            -- Aplicar velocidade MUITO ALTA no próprio corpo
            myHRP.AssemblyLinearVelocity = Vector3.new(9999, 9999, 9999)
            myHRP.AssemblyAngularVelocity = Vector3.new(9999, 9999, 9999)
            
            -- Atualizar posição do thrust
            if thrust and thrust.Parent then
                thrust.Location = myHRP.Position
            end
        end)
        
        -- Manter sem colisão para atravessar obstáculos
        Connections.BodyFlingNoclip = RunService.Stepped:Connect(function()
            if not State.BodyFlingAttack then return end
            local myHRP = GetHRP()
            if myHRP then
        ...
        Notify("Body Fling", "Atacando " .. targetPlayer.Name .. "!", 3)
        return true, "Body Fling ativado contra " .. targetPlayer.Name
    else
        ...
        local myHRP = GetHRP()
        if myHRP then
            local thrust = myHRP:FindFirstChild("_NDSBodyThrust")
...
local function CreateUI()
    ...
    Title.Text = "NDS Troll Hub v7.6 ANTI-FLING"
    ...
    CreateToggle("Body Fling", ToggleBodyFlingAttack, 6, "BodyFlingAttack")
    
    CreateCategory("PROTECAO", 10)
    CreateToggle("Anti-Fling", ToggleAntiFling, 11, "AntiFling")
    
    CreateCategory("UTILIDADES", 15)
    CreateToggle("View Player", ToggleView, 16, "View")
    CreateToggle("Speed 3x", ToggleSpeed, 17, "Speed")
    CreateButton("Fly GUI", ToggleFly, 18)
...
task.spawn(function()
    task.wait(1)
    Notify("NDS Troll Hub v7.6 ANTI-FLING", "Carregado!", 3)
end)

print("NDS Troll Hub v7.6 ANTI-FLING & BODY FLING - Carregado!")
